...
"""
Modification of the initial base_sp.py file from standard_precip library
"""
...
class BaseStandardIndex():
    ...
    @staticmethod
    def rolling_window_mean(df: pd.DataFrame, precip_cols: list, span: int=1, window_type: str=None,
        ...
        for p in precip_cols:
            ...
            df.loc[:,new_col_name] = df[p].rolling(
    ...
    def calculate(self, df: pd.DataFrame, date_col: str, precip_cols: list, indicator: str="spi", freq: str="M",
        ...
        if scale > 1:
            if indicator=="spi":
                ...
                df, precip_cols = self.rolling_window_mean(df, precip_cols, scale)
